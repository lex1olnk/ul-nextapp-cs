...
const columnMapping: Record<SortableColumn, keyof PlayerStats | ((player: PlayerStats) => number)> = {
  'Kills': 'kills',
  'Deaths': 'deaths',
  'Assists': 'assists',
...
export default function PlayersTable (props: {players: PlayerStats[]}) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.sortColumn) {
      result = [...result].sort((a, b) => {
        ...
        return filters.sortDirection === 'desc' ? valueA - valueB : valueB - valueA;
      });
    }
  ...
  const renderCellValue = (player: PlayerStats, column: SortableColumn) => {
    ...
    if (typeof value === 'function') {
      ...
      switch (column) {
        case 'HS%': return `${result.toFixed(0)}%`;
        case 'ADR': return result.toFixed(0);
        case 'KAST': return result.toFixed(0);
        case 'IMP': return (result * 1.75 + 0.1).toFixed(2);
        default: return result.toFixed(2);
      }
    }
            ...
            return (
              ...
              >
                <div className="grid-item">
                  {index == 0 && <Image className='flex absolute w-24 mix-blend-screen -translate-x-4 -translate-y-4' src={logo} alt="loading..." />}
                ...
                {Object.keys(columnMapping).map((column) => (
                  <div key={column} className="grid-item" style={{color: column == "Rating" ? getRatingColor(Number(renderCellValue(player, column as SortableColumn)), 0.3, 1.5):"#fff"}}>
